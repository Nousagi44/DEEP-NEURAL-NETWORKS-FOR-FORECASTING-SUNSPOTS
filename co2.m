...
%% ========================================================================
%  1) LOAD AND NORMALIZE CO2 DATA
%% ========================================================================

data = readmatrix('co2.csv','NumHeaderLines',1);
time   = data(:,1);
co2Val = data(:,2);

minVal  = min(co2Val);
maxVal  = max(co2Val);
co2ValNorm = 2*((co2Val - minVal)/(maxVal - minVal) - 0.5);

%% ========================================================================
%  2) CREATE 5-LAG INPUTS (X) AND TARGET (Y)
%% ========================================================================
inputDim = 5; 
N_total  = length(co2ValNorm);

X_all = zeros(N_total - inputDim, inputDim);
Y_all = zeros(N_total - inputDim, 1);

for i = 1:(N_total - inputDim)
    X_all(i,:) = co2ValNorm(i : i+inputDim-1);
    Y_all(i)   = co2ValNorm(i + inputDim);
end
...
%% ========================================================================
%  4) CNN WITH DROPOUT + MOMENTUM (3 hidden layers: 5,5,5)
%% ========================================================================
disp('=== CNN WITH DROPOUT (5,5,5) + MOMENTUM ===');

dropH1 = 0;       % no dropout on 1st hidden layer
dropH2 = 0.01;    % small dropout on 2nd hidden layer
dropH3 = 0;       % no dropout on 3rd hidden layer
...
plot_fit_train(time, inputDim, Y_all, Ypred_cnn_nodrop_all, Ntrain, ...
...
plot_fit_train_mlp(time, inputDim, Y_all, Ypred_mlp_noPrune_all, Ntrain, ...
    'MLP Single Hidden (No Prune) - Train Fit');

figure('Name','MLP No Pruning - Forecast','NumberTitle','off');
plot_one_step_forecast_solid_mlp(time, co2ValNorm, Xtest_all, Ytest_all, ...
...
plot_fit_train_mlp(time, inputDim, Y_all, Ypred_mlp_prune_all, Ntrain, ...
...
%% ========================================================================
%  8) Plot MSE Over Epochs (All 4 models)
%% ========================================================================
maxLen = max([
    length(MSE_cnn_drop), ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         BELOW ARE ALL THE HELPER FUNCTIONS IN THIS SINGLE SCRIPT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function weights = initialize_weights(nIn,nH1,nH2,nH3,initRange)
    % Creates a struct with random initial weights in [-initRange, +initRange]
    % w1, w2, w3, w4 fields.
    weights= struct();
    % w1
    for j=1:nH1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   TRAIN CNN WITH DROPOUT + MOMENTUM
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for ep=1:maxEpochs
    ...
    fprintf('CNN Dropout+Momentum Ep %3d/%3d: SSE=%.4f, MSE=%.4f\n', ...
...
for ep=1:maxEpochs
    ...
    for i=1:Ntrain
        ...
        [y_out,h1,h2,h3] = forward_pass(xp, weights, ...
            nIn,nH1,nH2,nH3);

        e = target - y_out;
        SSE= SSE+ e^2;

        grads    = backprop_deltas_noDrop(xp, target, weights, ...
                     h1,h2,h3, nIn,nH1,nH2,nH3);
        velocity = update_velocity(velocity, grads, learnRate, momentum);
        weights  = apply_velocity(weights, velocity);
    end
    ...
    MSE = SSE/Ntrain;
    MSE_over_epochs(ep)= MSE;
    fprintf('CNN NoDrop+Momentum Ep %3d/%3d: SSE=%.4f, MSE=%.4f\n', ...
            ep, maxEpochs, SSE, MSE);
    if SSE< tssLimit, break; end
end
MSE_over_epochs= MSE_over_epochs(1:ep);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   SINGLE-FUNCTION FORWARD PASS (NO DROPOUT)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   SINGLE-FUNCTION FORWARD PASS (DROPOUT + MOMENTUM)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   BACKPROP NO DROPOUT
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function grads = backprop_deltas_noDrop(xp, target, weights, ...
    h1,h2,h3, nIn,nH1,nH2,nH3)
grads= struct();
...
for i=1:(nH3+1)
    w4Name= sprintf('w4_%d', i);
    y_out= y_out+ weights.(w4Name)* h3_bias(i);
end
Error= target - y_out;
...
for i=1:(nH3+1)
    w4Name= sprintf('w4_%d', i);
    if i<=nH3
        grads.(w4Name)= Error * h3(i);
    else
        grads.(w4Name)= Error * 1;
    end
end
...
for jh3=1:nH3
    for ih2=1:(nH2+1)
        ...
        if ih2<=nH2
            grads.(w3Name)= delta_h3(jh3)* h2(ih2);
        else
            grads.(w3Name)= delta_h3(jh3)* 1;
        end
    end
end
...
for jh2=1:nH2
    for ih1=1:(nH1+1)
        ...
        if ih1<=nH1
            grads.(w2Name)= delta_h2(jh2)* h1(ih1);
        else
            grads.(w2Name)= delta_h2(jh2)* 1;
        end
    end
end
...
for jh1=1:nH1
    for i=1:(nIn+1)
        ...
        if i<=nIn
            grads.(w1Name)= delta_h1(jh1)* xp(i);
        else
            grads.(w1Name)= delta_h1(jh1)* 1;
        end
    end
end
end
...
for j=1:nH1
    ...
    for k=1:nH2
        w2Name= sprintf('w2_%d_%d', k,j);
        bperr= bperr+ delta_h3(k)* weights.(w2Name);
    end
    delta_h1(j)= (1- h1(j)^2)* bperr;
    if mask1(j)==0
        delta_h1(j)= 0;
...
for f=1:length(fNames)
    velocity.(fNames{f})= 0;  % start velocity at zero
end
end
...
for ep=1:maxEpochs
    ...
    fprintf('MLP NoPrune Ep %3d/%3d: SSE=%.4f, MSE=%.4f\n', ...
...
for ep=1:maxEpochs
    ...
    MSE_epoch= SSE/Ntrain;
    MSE_over_epochs(ep)= MSE_epoch;
    fprintf('MLP Prune Ep %3d/%3d: SSE=%.4f, MSE=%.4f\n', ...
        ep, maxEpochs, SSE, MSE_epoch);

    % Pruning step: set small weights to zero
    fieldNames= fieldnames(weights_mlp);
    for ff=1:length(fieldNames)
        arr= weights_mlp.(fieldNames{ff});
        arr(abs(arr)<0.001)= 0.0;   % threshold
...
function plot_fit_train(timeVec, nLags, Yall, YpredAll, Ntrain, plotTitle)
trainTimes = timeVec(nLags+1 : nLags+Ntrain);
trainActual= Yall(1:Ntrain);
trainPred  = YpredAll(1:Ntrain);

plot(trainTimes, trainActual, 'b','LineWidth',1.5); hold on;
plot(trainTimes, trainPred,   'r','LineWidth',1.5);
xlabel('Time'); ylabel('Normalized CO2');
title(plotTitle,'FontSize',12);
legend({'Actual','Predicted'},'Location','best');
grid on;
end

function plot_fit_train_mlp(timeVec, nLags, Yall, YpredAll, Ntrain, plotTitle)
trainTimes = timeVec(nLags+1 : nLags+Ntrain);
trainActual= Yall(1:Ntrain);
trainPred  = YpredAll(1:Ntrain);
...
function plot_one_step_forecast_solid(timeVec, dataNorm, ...
    Xtest, Ytest, ...
...
Ntest    = size(Xtest,1);
testTimes= timeVec(end-Ntest+1 : end);

hold on;
plot(testTimes, Ytest,'b','LineWidth',1.5,'DisplayName','Actual Test');
...
for i=1:Ntest
    ...
    if applyDrop
        [pred,~,~,~]= forward_pass_dropout_forMomentum(xp, weights, ...
            nIn,nH1,nH2,nH3, dropH1,dropH2,dropH3);
    else
...
plot(testTimes, yPredTest,'m','LineWidth',1.5,'DisplayName','Predicted Test');

lastTestTime= testTimes(end);
...
if applyDrop
    [forecastVal,~,~,~]= forward_pass_dropout_forMomentum([newWindow;1], ...
        weights, nIn,nH1,nH2,nH3, dropH1,dropH2,dropH3);
else
    [forecastVal,~,~,~]= forward_pass([newWindow;1], ...
        weights, nIn,nH1,nH2,nH3);
end

forecastTime= timeVec(end)+1;  % one step beyond the last time
plot([lastTestTime forecastTime], [lastTestPred forecastVal], ...
    'r','LineWidth',2,'DisplayName','One-step Forecast');

xlabel('Time'); ylabel('Normalized CO2');
title(plotTitle,'FontSize',12);
legend('show'); grid on;
xlim([testTimes(end)-5, forecastTime+1]);
end

function plot_one_step_forecast_solid_mlp(timeVec, dataNorm, ...
    Xtest, Ytest, ...
...
% SHIFT+APPEND => [ lastWindow(2:end); lastTestPred ]
newWindow= [ lastWindow(2:end); lastTestPred ];

forecastVal= singleMLP_forward([newWindow;1], weights_mlp, inDim, hidDim);
forecastTime= timeVec(end)+1;  % one step beyond
...
function y_out = singleMLP_forward(x_pattern, weights_mlp, inDim, hidDim)
    % Single hidden-layer MLP forward pass 
    % x_pattern: column vector [inDim + 1(bias)]
    net_h1= zeros(hidDim,1);
    for j=1:hidDim
        accum=0;
        for ii=1:(inDim+1)
